...
# ประเทศไทยไม่มี DST เลยใช้ offset คงที่ได้เลย ไม่ต้องพึ่ง tzdata
BKK = timezone(timedelta(hours=7))


def _is_past_reset(res_date_str):
    """โต๊ะจะรีเซ็ตอัตโนมัติตอนตี 1 ของคืนถัดจาก res_date แม้ยังไม่ได้เช็คบิล/ยกเลิก"""
    try:
        d = datetime.strptime(res_date_str, "%Y-%m-%d").date()
    except (TypeError, ValueError):
        return True
    cutoff = datetime(d.year, d.month, d.day, 1, 0, tzinfo=BKK) + timedelta(days=1)
    return datetime.now(BKK) >= cutoff
...
def checkout_reservation(res_id, paid_amount):
    ...
    ทำให้โต๊ะนี้ว่างให้จองใหม่ได้ทันที (taken_table_ids นับเฉพาะ status='confirmed')
    """
    try:
